...
const Login: React.FC<Props> = ({ onLogin, onNavigate, isAdminLogin = false }) => {
  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center px-4 animate-in fade-in duration-500">
      <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 max-w-md w-full relative overflow-hidden">
        
        {/* Decor */}
        <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-emerald-400 to-teal-600"></div>

        <div className="text-center mb-8">
           <div className="inline-flex p-3 bg-emerald-50 rounded-full text-emerald-600 mb-4">
        ...
        {isAdminLogin && (
          <div className="mt-6 text-center">
             <button onClick={() => onNavigate(ViewState.LOGIN)} className="text-xs text-slate-400 hover:text-slate-600">Back to User Login</button>
          </div>
        )}
      </div>
    </div>
